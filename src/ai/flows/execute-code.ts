...
const prompt = ai.definePrompt({
  ...
  prompt: `You are a machine. Your sole function is to simulate code execution. You MUST respond in JSON format matching the provided schema. Do not add any explanation.
      ...
      **CRITICAL SIMULATION RULES:**
      1.  **Infinite Loops:** If you detect an infinite or very long loop, simulate only a few (5-10) iterations of the loop's output. Then, set \`hasMoreOutput\` to \`true\` in your JSON response. This allows the simulation to continue in the next step.
      2.  **Program Termination:** When the program finishes execution, set \`hasMoreOutput\` to \`false\`.
      3.  **Waiting for Input:** When the program is waiting for user input, set \`isWaitingForInput\` to \`true\` and \`hasMoreOutput\` to \`false\`. The simulation will resume after input is provided.
      ...
      - **Task:**
        ...
        2.  Follow the **CRITICAL SIMULATION RULES** above.
        3.  Your JSON \`output\` field MUST contain only the *new* output generated by this step. Do NOT repeat any part of the previous transcript.
      ...
      - **Task:**
        ...
        4. **CRITICAL:** If the code is \`name = input("Enter name: ")\`, the \`output\` field must be \`"Enter name: "\`, \`isWaitingForInput\` MUST be \`true\`, and \`hasMoreOutput\` must be \`false\`.
